...
render also scaffolds a sibling `Models` class library holding the published
...
extractor's csproj references it as `../../Models/Models.csproj`; only the
inbound file shape (`In`) stays local to the component. The name is plain
`Models` because the project is scoped by the system directory it lives in.
